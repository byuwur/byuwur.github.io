...
<nav id="bywr-sidebar" class="bywr-sidebar accordion bywr-accordion bg-dark-transparent bg-blurred text-white">
  ...
  <div class="bywr-sidebar-content accordion-item flex-grow-1">
    <div class="bywr-sidebar-option p-2o5">
      <div class="navbar-brand has-background-contain" role="img" aria-label="<?= escape_html($LANG["sidebar.logo_alt"], false) ?>"
        style="height:48px;width:48px;background-image:url('<?= "{$HOME_PATH}/img/logo.png" ?>');"></div>
      <span class="ms-2 me-4 pe-5">[Mateus] byUwUr</span>
    </div>
    <a class="bywr-sidebar-option" href="#about" title="<?= escape_html($LANG["nav.about"], false) ?>" aria-label="<?= escape_html($LANG["nav.about"], false) ?>">
      <i class="fas fa-house"></i><span><?= $LANG["nav.about"] ?></span><i class="fas fa-angle-right ms-auto"></i>
    </a>
    <a class="bywr-sidebar-option" href="#resume" title="<?= escape_html($LANG["nav.resume"], false) ?>" aria-label="<?= escape_html($LANG["nav.resume"], false) ?>">
      <i class="fas fa-file-contract"></i><span><?= $LANG["nav.resume"] ?></span><i class="fas fa-angle-right ms-auto"></i>
    </a>
    <a class="bywr-sidebar-option" href="#contact" title="<?= escape_html($LANG["nav.contact"], false) ?>" aria-label="<?= escape_html($LANG["nav.contact"], false) ?>">
  ...
  <div class="bywr-sidebar-content accordion-item flex-grow-0">
    <!--button class="accordion-header accordion-button p-2o5 collapsed" data-bs-toggle="collapse" data-bs-target="#lang-drop" aria-expanded="false" aria-controls="lang-drop"
      title="<?= escape_html($LANG["language.selector"], false) ?>" aria-label="<?= escape_html($LANG["language.selector"], false) ?>">
    ...
    <div class="d-flex flex-row">
      <a class="bywr-sidebar-option" href="<?= "/{$ROUTE_ES}" ?>" title="<?= escape_html($LANG["language.spanish"], false) ?>" aria-label="<?= escape_html($LANG["language.spanish"], false) ?>"><img
          class="inline-logo" src="<?= "{$HOME_PATH}/img/co.svg" ?>" alt="<?= escape_html($LANG["resume.es.alt"], false) ?>" /> <?= $LANG["language.spanish"] ?><i class="fas fa-angle-right ms-auto"></i></a>
      <a class="bywr-sidebar-option" href="<?= "/{$ROUTE_EN}" ?>" title="<?= escape_html($LANG["language.english"], false) ?>" aria-label="<?= escape_html($LANG["language.english"], false) ?>"><img
          class="inline-logo" src="<?= "{$HOME_PATH}/img/gb.svg" ?>" alt="<?= escape_html($LANG["resume.en.alt"], false) ?>" /> <?= $LANG["language.english"] ?><i class="fas fa-angle-right ms-auto"></i></a>
    </div>
    <!--button class="accordion-header accordion-button p-2o5 collapsed" data-bs-toggle="collapse" data-bs-target="#theme-drop" aria-expanded="false" aria-controls="theme-drop"
      title="<?= escape_html($LANG["theme.selector"], false) ?>" aria-label="<?= escape_html($LANG["theme.selector"], false) ?>">
    ...
    <div class="d-flex flex-row">
      <a class="bywr-sidebar-option" href="javascript:document.getElementById('themesheet').setAttribute('href','css/v2.light.css');set_cookie('theme','light');" role="button"
        title="<?= escape_html($LANG["theme.light.title"], false) ?>" aria-label="<?= escape_html($LANG["theme.light.title"], false) ?>"><i class="fas fa-sun"></i><?= $LANG["theme.light"] ?><i
      ...
      <a class="bywr-sidebar-option" href="javascript:document.getElementById('themesheet').setAttribute('href','css/v2.dark.css');set_cookie('theme','dark');" role="button"
        title="<?= escape_html($LANG["theme.dark.title"], false) ?>" aria-label="<?= escape_html($LANG["theme.dark.title"], false) ?>"><i class="fas fa-moon"></i><?= $LANG["theme.dark"] ?><i
          class="fas fa-angle-right ms-auto"></i></a>
    </div>
  ...
  <a id="bywr-sidebar-toggle" class="bywr-sidebar-toggle" href="javascript:;" title="<?= escape_html($LANG["sidebar.toggle"], false) ?>" aria-label="<?= escape_html($LANG["sidebar.toggle"], false) ?>"
    data-bs-toggle="tooltip" data-bs-title="<?= escape_html($LANG["sidebar.toggle"], false) ?>"><i class="fas fa-bars"></i><span><?= $LANG["sidebar.menu"] ?></span></a>
  <div id="bywr-sidebar-hidden" class="bywr-sidebar-hidden">
    <div class="navbar-brand has-background-contain mt-auto" role="img" aria-label="<?= escape_html($LANG["sidebar.logo_alt"], false) ?>"
      style="height:48px;width:48px;background-image:url('<?= "{$HOME_PATH}/img/logo.png" ?>');"></div>
  </div>
</nav>
<?php
