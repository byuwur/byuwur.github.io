...
// --- PHP ---
$LANG = [];
if (file_exists("{$TO_HOME}/v1.nestux/lang.{$APP_LANG}.php")) require_once "{$TO_HOME}/v1.nestux/lang.{$APP_LANG}.php";
// Language fallbacks if lang is supported but file doesn't exist
$preferred_lang = substr($_SERVER["HTTP_ACCEPT_LANGUAGE"] ?? "es", 0, 2);
if (!$LANG && file_exists("{$TO_HOME}/v1.nestux/lang.{$preferred_lang}.php")) require_once "{$TO_HOME}/v1.nestux/lang.{$preferred_lang}.php";
if (!$LANG && file_exists("{$TO_HOME}/v1.nestux/lang.en.php")) require_once "{$TO_HOME}/v1.nestux/lang.en.php";
?>
...
<nav class="navbar navbar-expand-md navbar-dark bg-primary fixed-top p-3" id="sideNav">
  <a class="navbar-brand" href="#about">
    <span class="d-block d-md-none" style="color: #FFF;">
      <img src="<?= "{$HOME_PATH}/img/v1/nestux-favicon.png" ?>" height="36px" width="36px" style="margin-bottom: 4px;" alt="Logo" />
      <?= $_name_topbar ?>
    </span>
    <span class="d-none d-md-block">
      <img class="img-fluid img-profile rounded-circle mx-auto mb-2" src="<?= "{$HOME_PATH}/img/v1/nestux.jpg" ?>" alt="Marco" />
    </span>
  </a>
...
<div class="container old-container">
  <section id="about">
    ...
    <a href="#contact" class="a-lang text-uppercase mt-4 w-100"><?= $_start ?></a>
  </section>
  <section id="skills">
...
<script>
  ...
  $(() => {
    ...
    byCommon.init();
    if ($("#page-icon").length) $("#page-icon").attr("href", "<?= "{$HOME_PATH}/img/v1/nestux-favicon.png" ?>");
    if ($("#shortcut-icon").length) $("#shortcut-icon").attr("href", "<?= "{$HOME_PATH}/img/v1/nestux-favicon.png" ?>");
  });
</script>
<?php
